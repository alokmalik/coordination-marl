#s61 deliver without collison
import numpy as np
from multiagent.core import World, Agent, Landmark
from multiagent.scenario import BaseScenario

class Scenario(BaseScenario):
    def make_world(self, n_agents=2, use_dense_rewards=False, shuffle_landmarks=False, color_objects=True,
                   small_agents=False,network='survivalist'):
        world = World()
        ...
        self.agent_flag=np.zeros(num_agents)
        self.landmark_flag=np.zeros(num_landmarks)
    ...
    def reset_world(self, world):
        ...
        for n in range(num_landmarks):
            xcrds.append(-n/(num_landmarks-1))

        xcrds=np.array(xcrds)*.95
    ...
    def dense_reward(self, agent, world):
        ...
        if self.agent_flag[agent.i]:
            return rew
        else:
            dists = [np.sqrt(np.sum(np.square(agent.state.p_pos - l.state.p_pos))) for l in world.landmarks]
            
            for i,dist in enumerate(dists):
                if dist<agent.size and not self.agent_flag[agent.i] and not self.landmark_flag[i]:
                    self.agent_flag[agent.i]=1
    ...
    def reward(self,agent,world):
        ...
        if reward_type=='multiplicative':
            ...
            for i,reward in enumerate(personal_rewards):
                rew*=reward**network[i,agent.i]
        else:    
            #additive reward
            personal_rewards= np.matmul(personal_rewards,network)


            rew=personal_rewards[agent.i]
    ...
    def observation(self, agent, world):
        ...
        return np.concatenate([agent.state.p_vel] + [agent.state.p_pos] + entity_pos + other_pos + comm +[self.agent_flag]+ [self.landmark_flag])
